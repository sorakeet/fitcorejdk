        ...
        for(int k=left;k<right;run[count]=k){
            ...
            * */
            if(a[k]<a[k+1]){
                ...
                * 这里降序逻辑，如果当前是降序
                *  这里的 while 会直到
                *  判断是否是false，也就是开始升序，跳出处理
                * */
                while(++k<=right&&a[k-1]>=a[k]) ;
                /*
                * run[count]-1 运行时计数器 hi 存最大运行时索引，逆序处理
                * 得到的中间数组段是降序的
                * */
                for(int lo=run[count]-1, hi=k;++lo<--hi;){
                    /*
                    *  ++lo<--hi 这里相当于是对半交换，升序交换，结果是升序的
                    * */
                    int t=a[lo];
                    a[lo]=a[hi];
                    a[hi]=t;
                ...
                /*
                * 如果比较双方是相等的，需要预处理，特殊对待
                * */
                for(int m=MAX_RUN_LENGTH;++k<=right&&a[k-1]==a[k];){
                    /*
                    * ++k<=right&&a[k-1]==a[k] 这里就代表 在 MAX_RUN_LENGTH 计数范围内要求判决
                    * */
                    if(--m==0){
                        /*
                        * 如果在 MAX_RUN_LENGTH 计数范围内未判决 通过左侧胜出排序
                        * */
                        sort(a,left,right,true);
                        return;
                    }
            ...
            /*
             * 数组目前不是高度结构化
             * 如果未进入判断，使用快速排序而不是归并排序进行再排
             * 针对的是源数组 MAX_RUN_COUNT
             */
            if(++count==MAX_RUN_COUNT){
                /*
                * 这里的决策是对排序执行次数大于 MAX_RUN_COUNT 的数组源非快速排序
                * */
                sort(a,left,right,true);
                return;
            }
